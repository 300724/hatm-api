...
from hatm.application.repositories import SQLAlchemyHatmRepository, SQLAlchemyJuzRepository
from hatm.infrastructure.request_models import CreateHatmRequest, UpdateHatmRequest, CreateJuzRequest, UpdateJuzRequest
...
def list_hatms(creator_id: str = None, is_public: bool = None, is_completed: bool = None, is_published: bool = None, service: HatmService = Depends(get_hatm_service)):
    hatms = service.list_hatms(creator_id, is_public, is_completed, is_published)
    return hatms

@hatm_router.post("/", response_model=Hatm)
def create_hatm(request: CreateHatmRequest, service: HatmService = Depends(get_hatm_service)):
    hatm = service.create_hatm(request.creator_id, request.title, request.description, request.is_public, request.deadline)
    return hatm

@hatm_router.put("/{hatm_id}", response_model=Hatm)
def update_hatm(hatm_id: str, request: UpdateHatmRequest, service: HatmService = Depends(get_hatm_service)):
    hatm = service.update_hatm(hatm_id, request.title, request.description, request.is_public, request.is_completed, request.is_published, request.deadline)
    if not hatm:
        raise HTTPException(status_code=404, detail="Hatm not found")
    return hatm
...
def list_juzs(hatm_id: str = None, user_id: str = None, status: JuzStatus = None, service: JuzService = Depends(get_juz_service)):
    juzs = service.list_juzs(hatm_id, user_id, status)
    return juzs


@juz_router.post("/", response_model=Juz)
def create_juz(request: CreateJuzRequest, service: JuzService = Depends(get_juz_service)):
    juz = service.create_juz(request.hatm_id, request.user_id, request.juz_number, request.status, request.deadline)
    return juz

@juz_router.put("/{juz_id}", response_model=Juz)
def update_juz(juz_id: str, request: UpdateJuzRequest, service: JuzService = Depends(get_juz_service)):
